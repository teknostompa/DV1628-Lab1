...
void* professor(void* buf) {
    ...
    printProf(childID, "Thinking -> Pick up highest priority");
    pthread_mutex_lock(&locks[(left < right) ? left : right]); // här kollas prioriteten med ternär operator
    printProf(childID, "Pick up left -> Thinking");
    sleepRandom(2.0f, 8.0f);
    printProf(childID, "Thinking -> Pick up lowest priority");
    pthread_mutex_lock(&locks[(left < right) ? right : left]); // här kollas prioriteten med ternär operator
